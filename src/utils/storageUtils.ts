...
type LocalStorageValues<K extends LocalStorageKeys = LocalStorageKeys> =
  LocalStorageTypes[K];
...
export const getLocalStorageValueAndLog: GetLocalStorageAndLog = (
  key,
  defaultValue,
) => {
...
export const getShowCondensed = () =>
  getLocalStorageValueAndLog("condensedState", DEFAULT_CONDENSED_STATE);
export const getTextArray = () =>
  getLocalStorageValueAndLog("texts", DEFAULT_TEXTS);
export const getMostRecentText = () => getTextArray()[0];
export const getUserSettings = () => ({
  condensedState: getShowCondensed(),
});

export const storeCondensedState = (string: CondensedState) =>
  setLocalStorage("condensedState", string);
...
export const splitTitleIntoBookAndChapter = (
  ...
  const wordArray = title.split(" ") as
...
const movePassageToFrontOfArray = (
  title: string,
): TextsArray | typeof DEFAULT_TEXTS => {
  try {
